...
[System.Collections.Generic.List[object]]$runspaceTasks = @()
...
try {
    ...
    $totalWorkTime = [math]::Round((Measure-Command {
        ...
        while($runspaceTasks.Count -gt 0 -and $timeRemaining -gt 0) 
        {
	        foreach ($task in $runspaceTasks.ToArray()) 
            {
		        if ($task.AsyncResult.IsCompleted) 
                {
			        ...
			        $runspaceTasks.Remove($task)			
		        } 
	        } 
            $timeRemaining = $timeoutMilliseconds - $stopwatch.ElapsedMilliseconds
            Start-Sleep -Milliseconds 500
            Write-Host $runspaceTasks.Count -ForegroundColor Cyan
        } 
